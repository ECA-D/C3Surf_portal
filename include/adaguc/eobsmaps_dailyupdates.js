...
var defaultProjection = {srs:'EPSG:40000',bbox:'-1650000,-7000000,3500000,-1700000 '};
...
var yesterday =  function(date1){
    ...
    var diff = today - oneDayTimeStamp;
    var yesterdayDate = new Date(diff);
    var mm = yesterdayDate.getMonth()+1;
    if(mm<10){mm='0'+mm}
    var dd = yesterdayDate.getDate();
    if(dd<10){dd='0'+dd}
    var yesterdayString = yesterdayDate.getFullYear() + '-' + mm + '-' + dd+'T00:00:00Z';
...
var dimensionsUpdate = function(layer){
    ...
    previousSelectedDateIndex = dim.size()-1
    // begin Else
    if(dim.getValueForIndex(dim.size()-1) == yesterday(today)) {
	$("#yesterday").html("yesterday");
    }
    else {
//	var yesterdate = parseISO8601DateToDate(dim.getValueForIndex(dim.size()-1))
	var yesterday1 = "";
	var yesterday2;
	yesterday1 = dim.getValueForIndex(dim.size()-1);
	yesterday2 = parseISO8601DateToDate(yesterday1);
	$("#yesterday").html(yesterday2.toISO8601().split("T")[0]);
    }
    // end Else
    function update(index){
        ...
        $("#currenttime").html(date.toISO8601().split("T")[0]);
...
var updateLayerList = function(){
    ...
    for(var layernr=0;layernr<layers.length;layernr++){
		...
		request+="LAYERS=baselayer,"+URLEncode(layer.name)+",overlay&";
...
var adagucBeforeCanvasDisplay = function(ctx){
    if(!mainWebmapJS)return;
    
    let mx = 0;
    
    let mw=mainWebmapJS.getWidth() / 1;
    let mh = 6 + 15;
    ...
    if(activeLayer){
        ...
        var text = activeLayer.title;
        
        
        var timeDim = activeLayer.getDimension("time");
        if(timeDim){
            text+=" for ";
            text+=timeDim.currentValue.split("T")[0]
        }
        ctx.fillText(text,mx+5 ,my+10); 
    }
...
var newMap = function(element){
    ...
    webMapJS.setBaseLayers([baseLayer,overlay]);
    webMapJS.draw();
    
    
    webMapJS.addListener('beforecanvasdisplay', adagucBeforeCanvasDisplay, true);
    return webMapJS;
};
